...
import { LILDEX_PROGRAM_ADDRESS } from '../programs';
...
export type DecrementInstruction<
  TProgram extends string = typeof LILDEX_PROGRAM_ADDRESS,
  TAccountLildex extends string | AccountMeta<string> = string,
  TRemainingAccounts extends readonly AccountMeta<string>[] = [],
> = Instruction<TProgram> &
  InstructionWithData<ReadonlyUint8Array> &
  InstructionWithAccounts<
    [
      TAccountLildex extends string
        ? WritableAccount<TAccountLildex>
        : TAccountLildex,
      ...TRemainingAccounts,
    ]
  >;
...
export type DecrementInput<TAccountLildex extends string = string> = {
  lildex: Address<TAccountLildex>;
};

export function getDecrementInstruction<
  TAccountLildex extends string,
  TProgramAddress extends Address = typeof LILDEX_PROGRAM_ADDRESS,
>(
  input: DecrementInput<TAccountLildex>,
  config?: { programAddress?: TProgramAddress }
): DecrementInstruction<TProgramAddress, TAccountLildex> {
  // Program address.
  const programAddress = config?.programAddress ?? LILDEX_PROGRAM_ADDRESS;

  // Original accounts.
  const originalAccounts = {
    lildex: { value: input.lildex ?? null, isWritable: true },
  };
  ...
  const instruction = {
    accounts: [getAccountMeta(accounts.lildex)],
    programAddress,
    data: getDecrementInstructionDataEncoder().encode({}),
  } as DecrementInstruction<TProgramAddress, TAccountLildex>;
